        ...
        document.getElementById('upload-form').addEventListener('submit', function (e) {
            ...
            fetch('/', {
                ...
                .catch(error => {
                    ...
                    progressContainer.style.display = 'none';
                    showErrorNotification("Image Enhancement Failed", error.message || "An unexpected error occurred during image enhancement.");
                });
        });
        ...
        function startEventSource() {
            ...
            eventSource.onmessage = function (event) {
                ...
                if (data.status === 'downloading') {
                    ...
                    progressNotification.style.display = 'none';
                    downloadingModelNameElement.textContent = `Downloading Model Files`;
                    showErrorNotification("Model Initialization Error", `Failed to initialize model. Error: ${data.error_message}`);
                    eventSource.close();
                } else if (data === 'close') {
                    eventSource.close();
                }
            ...
            eventSource.onerror = function (event) {
                if (eventSource.readyState === EventSource.CLOSED) {
                    showErrorNotification("Model Download Issue", "Failed to establish connection for model download updates. Attempting to reconnect...");
